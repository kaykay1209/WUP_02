...
VitC is a numerical variable. cult is a categorical variable. 
so the helper file we need is numerical~Cat
...
```
the table shows that c39 is an average of 51.5 and the c52 is an average 64.4
...
```
The p-value is 0.00000003405 this is a very small number. what this number represents is the odds of getting a sample like the one we just
examined if the null hypothesis is TRUE. I reject the Null hypothesis.
...
My conclusion is that the relationship between c52 and c39 is that c52 has more vitamin c content and that c39 has less 